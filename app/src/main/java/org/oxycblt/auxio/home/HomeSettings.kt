...
class HomeSettingsImpl @Inject constructor(@ApplicationContext context: Context) :
    ...
    override fun migrate() {
        ...
        // One-time: put the playlists tab first and ensure it is visible.
        if (!sharedPreferences.getBoolean(KEY_PLAYLISTS_FIRST_MIGRATED, false)) {
            val tabs = homeTabs.toMutableList()
            val playlistIndex = tabs.indexOfFirst { it.type == MusicType.PLAYLISTS }
            if (playlistIndex >= 0) {
                tabs.removeAt(playlistIndex)
                tabs.add(0, Tab.Visible(MusicType.PLAYLISTS))
                L.d("Migrating playlists tab to first position: $tabs")
                sharedPreferences.edit {
                    putInt(getString(R.string.set_key_home_tabs), Tab.toIntCode(tabs.toTypedArray()))
                    putBoolean(KEY_PLAYLISTS_FIRST_MIGRATED, true)
                }
            } else {
                sharedPreferences.edit { putBoolean(KEY_PLAYLISTS_FIRST_MIGRATED, true) }
            }
        }

        // One-time: insert Folders tab (visible) after Playlists.
        if (!sharedPreferences.getBoolean(KEY_FOLDERS_TAB_MIGRATED, false)) {
            val tabs = homeTabs.toMutableList()
            if (tabs.none { it.type == MusicType.FOLDERS }) {
                val insertAt =
                    (tabs.indexOfFirst { it.type == MusicType.PLAYLISTS } + 1).coerceAtLeast(0)
                tabs.add(insertAt, Tab.Visible(MusicType.FOLDERS))
                L.d("Migrating folders tab into configuration: $tabs")
                sharedPreferences.edit {
                    putInt(getString(R.string.set_key_home_tabs), Tab.toIntCode(tabs.toTypedArray()))
                }
            }
            sharedPreferences.edit { putBoolean(KEY_FOLDERS_TAB_MIGRATED, true) }
        }
    }
    ...
    companion object {
        ...
        private const val KEY_PLAYLISTS_FIRST_MIGRATED = "auxio_playlists_tab_first_migrated"
        private const val KEY_FOLDERS_TAB_MIGRATED = "auxio_folders_tab_migrated"
    }
}

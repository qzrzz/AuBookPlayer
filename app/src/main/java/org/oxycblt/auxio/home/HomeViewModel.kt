...
constructor(
    ...
    private val _folderList = MutableStateFlow(listOf<SongFolder>())
    /** A list of [SongFolder]s, sorted by the preferred [Sort], to be shown in the home view. */
    val folderList: StateFlow<List<SongFolder>>
        get() = _folderList
    ...
    /** Look up a [SongFolder] by its stable [SongFolder.key]. */
    fun folderByKey(key: String): SongFolder? = folderList.value.find { it.key == key }

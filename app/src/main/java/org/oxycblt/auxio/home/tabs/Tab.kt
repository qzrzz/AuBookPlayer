...
sealed class Tab(open val type: MusicType) {
    ...
    companion object {
        ...
        /**
         * The default tab sequence, in integer form. Visible tabs ordered as "Playlists",
         * "Folders", "Songs", "Albums", "Artists", with "Genres" hidden.
         */
        // VTTT chunks: playlists, folders, songs, albums, artists, genres(hidden)
        // 1100 1101 1000 1001 1010 0011
        const val SEQUENCE_DEFAULT = 0b1100_1101_1000_1001_1010_0011
        ...
        fun fromIntCode(intCode: Int): Array<Tab>? {
            ...
            // Ensure every known mode is present (pad missing ones as invisible).
            // This keeps older 5-tab sequences valid after FOLDERS was introduced.
            val present = distinct.map { it.type }.toSet()
            val complete = distinct.toMutableList()
            for (mode in MODE_TABLE) {
                if (mode !in present) {
                    complete += Invisible(mode)
                }
            }

            if (complete.isEmpty()) {
                L.e("Sequence size was empty, which is invalid")
                return null
            }

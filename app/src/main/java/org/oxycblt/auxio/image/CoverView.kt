...
constructor(context: Context, attrs: AttributeSet? = null, @AttrRes defStyleAttr: Int = 0) :
    ...
    @Inject lateinit var albumCoverStore: AlbumCoverStore
     ...
     * @param fallbackPlaylist When [Song.cover] is null and this playlist is provided, the
     *   playlist's cover (stacked art or generated text cover) is used instead.
     */
    fun bind(song: Song, fallbackPlaylist: Playlist? = null) =
        bindImpl(
            { size ->
    ...
    fun bind(album: Album) {
        // Priority: custom user cover → embedded song cover → generated text cover.
        bindImpl(
            {
                resolveAlbumCoverData(
                    context,
                    album,
                    backgroundColor(),
                    context.coverForegroundColor(),
                    customCover = albumCoverStore.getCustomCover(album.uid),
                )
            },
    ...
    fun bind(playlist: Playlist) =
        ...
        bindImpl(
            { size ->
                resolvePlaylistCoverData(
                    context,
                    playlist,
                    responsiveCornerRatio(size),
                    backgroundColor(),
                    context.coverForegroundColor(),
                    customCover = playlistCoverStore.getCustomCover(playlist.uid),
                )
            },

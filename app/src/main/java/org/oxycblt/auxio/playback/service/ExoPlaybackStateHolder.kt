...
import org.oxycblt.auxio.playback.PlaylistSkipSettings
import org.oxycblt.auxio.playback.SkipHeadTail
...
class ExoPlaybackStateHolder(
    ...
    private val skipJob = SupervisorJob()
    private val skipScope = CoroutineScope(Dispatchers.Main.immediate + skipJob)
    private var currentSaveJob: Job? = null
    private var openAudioEffectSession = false
    private var skipTailWatchJob: Job? = null
    /** Guards against re-entrant next() when auto-skipping the tail. */
    private var skippingTail = false
        ...
        skipScope.launch {
            playlistSkipSettings.updates.collect { uid ->
                ...
                if (playlist != null && playlist.uid == uid) {
                    L.d("Skip settings updated for current playlist, reapplying")
                    applySkipHeadIfNeeded()
                    startSkipTailWatch()
                }
            }
        }
    ...
    override fun applySavedState(
        ...
        // Restored position is respected; only re-apply head if we landed near the start.
        applySkipHeadIfNeeded()
        startSkipTailWatch()
    ...
    override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
        ...
        // Fresh start of a track (auto-advance, skip next/prev, playlist change): apply head skip.
        when (reason) {
    ...
    /** Current skip settings for the active playlist parent, or [SkipHeadTail.NONE]. */
    private fun currentSkip(): SkipHeadTail {
        val playlist = parent as? Playlist ?: return SkipHeadTail.NONE
        return playlistSkipSettings.get(playlist.uid)
    }

    /**
     * Resolve effective skip windows for a song, disabling skips when the track is too short to
     * leave a playable middle section.
     */
    private fun resolveSkip(song: Song?): SkipHeadTail {
        if (song == null) return SkipHeadTail.NONE
        val skip = currentSkip()
        if (!skip.isActive) return SkipHeadTail.NONE
        // Require at least 1 second of playable content after both skips.
        if (song.durationMs <= skip.headMs + skip.tailMs + 1000L) {
            L.d("Song too short for skip settings, ignoring [$song duration=${song.durationMs}]")
            return SkipHeadTail.NONE
        }
        return skip
    }
    ...
    /**
     * Watch playback position and advance when we enter the configured tail skip region.
     * Cancelled and restarted on every track change / settings update.
     */
    private fun startSkipTailWatch() {
        skipTailWatchJob?.cancel()
        skipTailWatchJob =
            skipScope.launch {
                while (true) {
                    ...
                    val skip = resolveSkip(song)
                    if (song != null && skip.tailMs > 0L && player.isPlaying) {
                        val endAt = song.durationMs - skip.tailMs
                        if (player.currentPosition >= endAt && !skippingTail) {
                            L.d("Reached tail skip region, advancing from $song")
                            skippingTail = true
                            try {
                                ...
                                // Small delay so we don't re-trigger on the same frame.
                                delay(300)
                                skippingTail = false
                            }
                        }

...
class PlaybackBarFragment : ViewBindingFragment<FragmentPlaybackBarBinding>() {
    ...
    override fun onBindingCreated(
        ...
        // -- VIEWMODEL SETUP ---
        collectImmediately(playbackModel.song, playbackModel.parent, ::updateSong)
    ...
    private fun onPlaylistCoverUpdated(uid: Music.UID) {
        val parent = playbackModel.parent.value as? Playlist
        val song = playbackModel.song.value
        if (song != null && parent != null && parent.uid == uid) {
            updateSong(song, parent)
        }
    }
    ...
    private fun updateSong(song: Song?, parent: MusicParent?) {
        if (song == null) {
            // Nothing to do.
            return
        }
        ...
        binding.playbackCover.bind(song, parent as? Playlist)

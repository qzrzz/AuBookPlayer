...
class PlaylistMenuDialogFragment : MenuDialogFragment<Menu.ForPlaylist>() {
    ...
    @javax.inject.Inject lateinit var playlistCoverStore: org.oxycblt.auxio.image.PlaylistCoverStore
    ...
    override fun getDisabledItemIds(menu: Menu.ForPlaylist): Set<Int> {
        ...
        // Only allow clearing a cover when a custom one is set.
        if (!playlistCoverStore.hasCustomCover(menu.playlist.uid)) {
            disabled += R.id.action_clear_cover
        }
        return disabled
    }

...
class PlaylistDetailFragment :
    ...
    override fun onShuffleParent(parent: Playlist) {
        // Playlist detail reuses the shuffle control as "last progress" / continue.
    ...
    private fun updatePlaylist(playlist: Playlist?, editedPlaylist: List<Song>?) {
        ...
        playlistListAdapter.setPlaylist(playlist)
        ...
        // Replace shuffle with "last progress" (continue) for playlists / audiobooks.
        binding.detailShuffleButton?.apply {
            ...
            setOnClickListener { playbackModel.playFromLastProgress(playlist) }
        }
        configurePlaylistToolbarContinue()
    ...
    private fun configurePlaylistToolbarContinue() {
        // AuxioToolbar renders action items as MaterialButtons (no standard Menu API).
    ...
    private fun handlePlaylistDecision(decision: PlaylistDecision?) {
        ...
        val directions =
            when (decision) {
                ...
                is PlaylistDecision.SetCover -> {
                    L.d("Setting cover for ${decision.playlist}")
                    pendingCoverPlaylist = decision.playlist
    ...
    private fun handlePlaylistMessage(message: PlaylistMessage?) {
        ...
        // Refresh playlist header + song row covers after set/clear. Songs without album art
        // inherit the playlist cover, so they must rebind when the custom cover changes.
        if (
            message is PlaylistMessage.CoverSetSuccess ||
                message is PlaylistMessage.CoverCleared
        ) {
            detailModel.currentPlaylist.value?.let { playlist ->
                requireBinding().detailCover.bind(playlist)
                playlistListAdapter.refreshCovers()
            }
        }
    }

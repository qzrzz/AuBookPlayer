...
constructor(
    ...
    private val _currentFolder = MutableStateFlow<SongFolder?>(null)

    /** The current [SongFolder] to display. Null if there is nothing to show. */
    val currentFolder: StateFlow<SongFolder?>
        get() = _currentFolder
    ...
    /**
     * The [PlaySong] instructions to use when playing a [Song] from [SongFolder] details.
     * Folders are not music parents, so fall back to playing the song by itself.
     */
    val playInFolderWith: PlaySong
        get() = PlaySong.ByItself
    ...
    override fun invalidate(type: MusicType, replace: Int?) {
        when (type) {
            ...
            MusicType.FOLDERS -> {
                refreshFolder(currentFolder.value?.key ?: return, replace)
            }
    ...
    /**
     * Navigate to the details of a [SongFolder].
     *
     * @param folder The [SongFolder] to navigate with.
     */
    fun showFolder(folder: SongFolder) = showImpl(Show.FolderDetails(folder))
    ...
    /**
     * Set a new [currentFolder] from its stable key. [currentFolder] and [folderSongList] will be
     * updated to align with the new folder.
     *
     * @param key The [SongFolder.key] of the folder to show. Must be valid.
     */
    fun setFolder(key: String) {
        L.d("Opening folder $key")
        if (key == _currentFolder.value?.key) {
            return
        }
        refreshFolder(key, null)
        if (_currentFolder.value == null) {
            L.w("Given folder key was invalid")
        }
    }
    ...
    private fun refreshFolder(key: String, replace: Int?) {
        L.d("Refreshing folder list for $key")
        val library = musicRepository.library
        if (library == null) {
            _currentFolder.value = null
            return
        }
        val folder = SongFolder.fromSongs(library.songs).find { it.key == key }
        if (folder == null) {
            _currentFolder.value = null
            return
        }
        ...
        val instructions =
            if (replace == -1) {
                // Keep the sort header and replace the song items after it.
                UpdateInstructions.Replace(1)
            } else {
                UpdateInstructions.Diff
            }
        _folderSongInstructions.put(instructions)
        _folderSongList.value = list
        _currentFolder.value = SongFolder(folder.path, sortedSongs)
    }
}
...
sealed interface Show {
     ...
     * @param folder The [SongFolder] to navigate with.
     */
    data class FolderDetails(val folder: SongFolder) : Show
}

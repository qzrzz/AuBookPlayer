...
class PlaylistDetailListAdapter(private val listener: Listener) :
    ...
    /**
     * Provide the playlist currently being displayed so song rows can fall back to its cover.
     */
    fun setPlaylist(playlist: Playlist?) {
        if (this.playlist == playlist) return
        this.playlist = playlist
        // Covers depend on the playlist fallback; rebind song rows.
        notifyItemRangeChanged(0, currentList.size)
    }

    /**
     * Rebind song rows so they pick up a newly set/cleared custom playlist cover (songs without
     * album art inherit the playlist cover).
     */
    fun refreshCovers() {

...
constructor(
    ...
    /**
     * Emits a playlist [Music.UID] whenever that playlist's custom cover is set or cleared. UI that
     * shows song art with a playlist-cover fallback should rebind when this fires.
     */
     ...
     */
    /**
     * Prompt the user to pick an image as the custom cover for [playlist], or apply [uri] if
     * already chosen.
     */
    fun setPlaylistCover(playlist: Playlist, uri: Uri? = null) {
        if (uri != null) {
            viewModelScope.launch {
                val ok = playlistCoverStore.setCover(playlist.uid, uri)
                _playlistMessage.put(
                    if (ok) PlaylistMessage.CoverSetSuccess else PlaylistMessage.CoverSetFailed
                )
            }
        } else {
            L.d("Launching cover picker for $playlist")
            _playlistDecision.put(PlaylistDecision.SetCover(playlist))
        }
    }

    /** Remove any custom cover for [playlist], restoring the auto-generated cover. */
    fun clearPlaylistCover(playlist: Playlist) {
        viewModelScope.launch {
            playlistCoverStore.clearCover(playlist.uid)
...
sealed interface PlaylistMessage {
    ...
    data object CoverSetSuccess : PlaylistMessage {
        override val stringRes: Int
            get() = R.string.lng_playlist_cover_set
    }

    data object CoverSetFailed : PlaylistMessage {
        override val stringRes: Int
            get() = R.string.err_playlist_cover_failed
    }

    data object CoverCleared : PlaylistMessage {
        override val stringRes: Int
            get() = R.string.lng_playlist_cover_cleared
    }
}
